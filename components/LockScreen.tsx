...
export const LockScreen: React.FC<LockScreenProps> = ({ onUnlock }) => {
  ...
  if (showOtpScreen) {
    return (
      <div className="fixed inset-0 z-[500] bg-emerald-950 flex flex-col items-center justify-center p-8 text-white text-center animate-in fade-in duration-500">
        ...
        <h1 className="text-3xl font-black mb-2 tracking-tight uppercase leading-tight">License<br/>Activated!</h1>
        <p className="text-emerald-100/60 mb-8 max-w-xs mx-auto text-sm leading-relaxed">
          Your Temporary Admin PIN is shown below. You must use it once to set your secret permanent PIN.
        </p>
        ...
        <div className="w-full max-w-sm bg-white/5 border border-white/10 p-10 rounded-[48px] mb-8 relative overflow-hidden shadow-2xl">
          ...
          <p className="text-emerald-500/40 text-[10px] font-black uppercase tracking-[0.4em] mb-4">One-Time Password (OTP)</p>
        ...
        >
          Proceed to Setup PIN <ArrowRight size={18} />
        </button>

        <p className="mt-8 text-[9px] text-emerald-500/30 font-black uppercase tracking-[0.3em]">
          NaijaShop Secure Setup Layer
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-[400] bg-emerald-950 flex flex-col items-center justify-center p-6 text-white text-center overflow-y-auto">
      ...
      <h1 className="text-3xl font-black mb-2 tracking-tight uppercase">Enter License</h1>
      <p className="text-emerald-100/60 mb-8 max-w-xs mx-auto text-sm">
        Activation required to use the POS. Send your Request Code to get a key.
      </p>
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <a 
          href={`https://api.whatsapp.com/send?phone=2347062228026&text=Request%20Code:%20${requestCode}`} 
          target="_blank"
        ...
        <div className="relative pt-4">
          ...
          <div className="relative flex justify-center text-[10px] uppercase font-black tracking-widest bg-emerald-950 px-3 text-emerald-700">Enter Activation Key</div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <button onClick={handleActivate} disabled={isVerifying || !activationKey} className="w-full bg-white text-emerald-900 font-black py-4 rounded-[24px] hover:bg-emerald-50 transition-all uppercase tracking-widest text-xs">
            {isVerifying ? 'Verifying...' : 'Verify & Unlock'}
          </button>
          {error && <p className="text-red-400 text-[10px] font-black uppercase tracking-widest animate-bounce mt-2 text-center w-full">Invalid Activation Key</p>}
        </div>
      </div>
    </div>
  );
